...
class TextManipulator
{
    ...
    {
        $file = \ForceUTF8\Encoding::toUTF8($file);
     ...
     */
    public function modify(array $transformation, $debug = false)
    {
        ...
        if (($transformation['marker']['source'] == 'a' && $transformation['type'] != static::MODIFY_COMMON)
            ...
            $modified = \ForceUTF8\Encoding::toUTF8(trim($this->read($transformation['length'], $transformation['start'] - $this->offset)));

            if (trim($modified)) {

                $pattern = '<li><a href="#%5$s%1$s_%2$s" id="l%4$s%1$s_%2$s" ';
                $pattern.= 'data-tags="'.$transformation['tags'].'" ';
    ...
    public function nl2br()
    {
        ...
        return file_put_contents($this->_modified->getPathname(), $content);
    }
    ...
    public function pageBreakAt($pageBreak, $htmlReplacement, $after = false)
    {
        ...
        $arrayPageBreak = array_map(function($item) {
        ...
        $nb = 0;
        $result = preg_replace('`' . implode('(<[^>]*?>| |\r?\n){0,}', $arrayPageBreak) . '`', $htmlReplacement, $content, -1, $nb);
        if ($nb === 0) {
            $table = array(
                'È' => 'È', '–' => '–', '—' => '–'
            );

            $content2 = strtr($content, $table);
            $content2 = preg_replace("/\r|\n/", "", $content2);


            $tableR = ['.' => '\.', '^' => '\^', '–' => '-', '?' => '\?'];
            foreach ($tableR as $key => $item) {
                foreach ($arrayPageBreak as $apb => $itemApb) {
                    $arrayPageBreak[$apb] = str_replace($key, $item, $itemApb);
                }
            }

            $replaceV = strtr(implode('(<[^>]*?>| |\r?\n){0,}', $arrayPageBreak), $table);
            $replaceV = str_replace('-', '(<[^>]*?>|-|\r?\n){0,}', $replaceV);
            $result = (preg_replace('`' . $replaceV . '`', $htmlReplacement, $content2, 1, $nb));

            if ($nb === 0) {
                var_dump("<br /><br />PAGE NOT FOUND<br />");
                var_dump(htmlentities($htmlReplacement));
                var_dump("<br /><br />");
                var_dump($arrayPageBreak);
                var_dump("<br /><br />");
                var_dump($replaceV);
            }
        }
    ...
    private function getOpenPattern($transformation)
    {
        switch ($transformation['type']) {
            ...
            case static::MODIFY_REPLACED:
                $pattern = '<a href="#%4$s%1$s_%2$s" class="span_%1$s sync" id="%3$s%1$s_%2$s" data-tags="%5$s" class="sync sync-single">';
                break;
            default:
                $pattern = '<span class="span_%1$s" id="%3$s%1$s_%2$s" data-tags="%5$s">';
                break;
        }
        return sprintf(
            $pattern,
            ...
            $transformation['marker']['source'],
            $transformation['marker']['target'],
            isset($transformation['tags']) ? $transformation['tags'] : ''
        );
    }
