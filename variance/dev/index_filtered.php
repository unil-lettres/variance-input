                    ...
                    <div class="catalogue__item row">
						...
						<?php if ($element['w_id'] != $previous['w_id']): ?>
                            <div class="col-sm-8">
								...
								<?php $query = 'SELECT c.id as c_id, c.number as c_number, c.folder AS c_folder, c.publication_scope AS c_scope, s.name as s_name, t.name AS t_name FROM comparisons c INNER JOIN versions s ON c.source_id = s.id INNER JOIN versions t ON c.target_id = t.id WHERE s.work_id = :id ORDER BY CASE WHEN COALESCE(c.sort_order, c.number) IS NULL THEN 1 ELSE 0 END, COALESCE(c.sort_order, c.number) ASC, c.id ASC';
								...
								if (!empty($comparisons)):
                                    ...
                                    $url = getOeuvreUrl($element['a_folder'], $element['w_folder'], $version['c_id']); ?>
                                    ...
                                    <ul class="catalogue-versions">
										...
										for ($i = 1; $i < count($comparisons); $i++):
                                            ...
                                            <li><a href="<?php echo getOeuvreUrl($element['a_folder'], $element['w_folder'], $version['c_id']); ?>"><?php echo '<span>'. $version['c_number'].'. '.$version['s_name'] . ' </span><span class="arrow-versions">&rarr;</span> ' . $version['t_name']; ?></a></li><?php
										endfor; ?>
                                    </ul>
